...
use App\Models\Message\InboundMeta as InboundMessageMeta;
use App\Models\Message\OutboundMeta as OutboundMessageMeta;
...
class Outbound extends Model
{
    ...
    {
        return $this->hasOne('App\Models\Message\OutboundMeta');
    }
     ...
     * @return @void
     */
    public function trigger (InboundMessageMeta $meta)
    {
        $handle  = $meta->handle;
        ...
        $this->medium_id     = $message->medium_id;
        $this->to            = $message->from;
        $this->from          = $message->to;

        // set meta
        $m                = new OutboundMessageMeta;
        $m->handle_id     = $meta->handle_id;
        $m->contact_id    = $meta->contact_id;
        $m->mechanism_id  = $meta->mechanism_id;

        // check that we can send the message
        if (!$this->canSend($handle, $message->id, $message->body)) {
            return;
        }

        // compile message content
        $this->body = $this->compileMessageBody($message->body);
        if (empty($this->body)) {
            return;
        }

        $m->message_id = Helpers::sms($handle->handle, $this->to, $this->body);
        try {
            $this->save();
        } catch (\Exception $e) {
        ...
        $m->outbound_id = $this->id;
        try {
            $m->save();
        } catch (\Exception $e) {
            $subject = 'OutboundMeta model save failed!';
            $error   = __FILE__ . ' @ ' . __LINE__ . ': ' . $e->getMessage();
            $meta    = ['record' => $this];
            Notification::send(User::first(), new Slack($subject, $error, $meta));
            return;
     ...
     * @return boolean
     */
    protected function canSend ($handle, $inbound_id, $inbound_body)
    {
        $send = true;
        ...
        // check handle status
        if ($handle->status_id != Status::ACTIVE) {
